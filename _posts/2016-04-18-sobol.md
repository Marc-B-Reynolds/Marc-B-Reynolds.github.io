...
\\
Uniform sampling of (open) unit disk ($\mathbb{D}^1$) and half-disk ($x\geq 0$) are performed using a rejection method.  This has no impact on the well-space properties.  The average number of iterations is $\frac{4}{\pi} \approx 1.27324$.  They both take a 2D generator $s$ and place the point in $p$.  The returned value is $p \cdot p$.
...
{% highlight c %}
float sobol_uniform_s2(sobol_2d_t* s, float* p);
float sobol_uniform_hs2(sobol_2d_t* s, float* p);
{% endhighlight %}
