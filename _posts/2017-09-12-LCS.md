...
First we can define the *multiplicative order*[^webmo] of $a$ as the smallest positive integer $t$ such that $\left(a^t\right)\bmod m = 1$ which is common written as:
...
$$ \left\{B\right\} = \left\{y_0,~y_0,\ldots,~y_{t-1}\right\} $$
...
\\
The bolded zeroes show the first element of the cycle for that bit position. These are always zero since since by defination $f_0 = 0$ and we are back to that position every $2^n$ steps. And the bottom $n$ bits for 1-5 (2 digit hex) and starting from zero this time:
