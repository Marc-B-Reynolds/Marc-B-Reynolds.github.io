...
WIP: There are a few bits-and-bobs missing
</div>
...
The mixing function is additionally never called directly it is instead a macro expansion which defaults to:  `#define PRNS_MIX(X) prns_mix(X)`
...
* Single state of 64-bits per generator.
* Both the main and down sequences have periods of 2<sup>64</sup>.

<div class="alert alert-warning" role="alert">
TODO: I need to all the smallcrush and crush results and probably some comments on choosing constants.  
</div>
------
...
The basics are pretty straight forward and are implemented in: ([lcgs.h](http://github.com/Marc-B-Reynolds/Stand-alone-junk/blob/master/src/SFH/lgcs.h))

<div class="alert alert-warning" role="alert">
Also this can be generalized to other structures...but no interesting choice other than LCGs are coming to mind.
</div>
...
[^weylOverview]: Weyl sequence overview [post]({{site.base}}/math/2016/02/24/weyl.html)
